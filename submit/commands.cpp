...
//********************************************
#include "commands.h"
#include <sys/stat.h>	// Used for makedir(2)
using namespace std;
...
typedef struct vars
{
    ...
    /* Consructor for easier use of iterators and find() function of STL*/
...
//**************************************************************************************
list<job>::iterator find_job(int num)
{
...
int ExeCmd(char* lineSize, char* cmdString)
{
    ...
    for (i = 1 ; i <= num_arg ; i++)
    {
        char* next_arg = (char*)malloc (sizeof (char)*(s_args[i].length()+1));
        if (next_arg == NULL)
        {
            fprintf(stderr, "smash error: > out of memory\n");
            exit(1);
        }
        strcpy(next_arg, s_args[i].c_str());
        args[i] = next_arg;
    }
    args[i] = NULL;
...
/*************************************************/
    ...
    else if (!strcmp(cmd, "show"))
	{
        ...
        else
        {
            ...
            if (it == shell_vars.end())
            {
                fprintf(stderr, "smash error: > variable doesn't exist \n");
                return 1;
            }
            else
            {
...
void ExeExternal(char *args[MAX_ARG], char* cmdString)
{
    ...
    switch(pID = fork()) 
	{
			...
			default:	// Parent Process //
                    ...
                    fg_job = &new_job;
                    int i=1;
                    while (args[i] != NULL) // free allocated memory
                    {
                        free(args[i]);
                        i++;
                    }
